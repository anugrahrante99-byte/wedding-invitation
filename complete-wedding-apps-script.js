/**
 * COMPLETE WEDDING INVITATION GOOGLE APPS SCRIPT
 * 
 * Features:
 ...
 * 5. Run setupWeddingSheets() to create sheets
 * 6. Deploy as Web App with "Who has access: Anyone"
 * 7. Copy Web App URL and update frontend files
 */
...
/**
 * =======================================================
 * MAIN API HANDLERS WITH CORS HEADERS
 * =======================================================
 */

/**
 * GET Request Handler
 * Handles all read operations with proper CORS headers
 */
function doGet(e) {
  try {
 ...
 * Handles all write operations with proper CORS headers
 */
function doPost(e) {
  try {
...
/**
 * MAIN SETUP FUNCTION
 * Run this function to setup complete wedding management system
 */
function setupWeddingSheets() {
  try {
    ...
    Logger.log("3. Copy Web App URL and update your frontend files");
...
/**
 * Clear all data from sheets (for testing)
 */
    ...
    Object.values(SHEETS).forEach(sheetName => {
      ...
      if (sheet) {
        ...
        if (lastRow > 1) {
          ...
          Logger.log("🗑️ Cleared data from: " + sheetName);
        }
      }
    });
    
    SpreadsheetApp.getUi().alert("✅ All data cleared (headers preserved)");
    
  } catch (error) {
    Logger.log("Clear data error: " + error.toString());
    SpreadsheetApp.getUi().alert("Clear data failed: " + error.toString());
  }
}
...
function addSampleData() {
  try {
    ...
    // Sample sent invitation
    markSent({
      name: "Jane Smith",
      phone: "628123456789",
      link: "https://anugrahrante99-byte.github.io/wedding-invitation/"
    });
    ...
    SpreadsheetApp.getUi().alert("✅ Sample data added for testing");
    
  } catch (error) {
    Logger.log("Add sample data error: " + error.toString());
    SpreadsheetApp.getUi().alert("Add sample data failed: " + error.toString());
  }
}
