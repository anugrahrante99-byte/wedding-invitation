/**
 * ULTIMATE JSONP WORKING SCRIPT
 * 
 * Complete functionality dengan JSONP instead of CORS
 * No setHeader calls - uses JSONP for all requests
 */
...
function doGet(e) {
  try {
    ...
    // ALWAYS return JSONP for consistency
    if (callback) {
...
function doPost(e) {
  try {
    ...
    const callback = body.callback || 'callback';
    ...
    // Return JSONP for POST requests
    return ContentService.createTextOutput(callback + '(' + jsonResponse + ')')
      .setMimeType(ContentService.MimeType.JAVASCRIPT);
